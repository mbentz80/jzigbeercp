/*
    ...
    $Id: clip.c 685 2003-03-08 15:26:51Z travo $
...
int ClipBegin(const GpReal* xx, const GpReal* yy, int nn, int clsd)
{
  ...
  if (n+1>maxwork) {
    ClipFreeWS();
    maxwork= n+256;
    xwork= (GpReal*)p_malloc(sizeof(GpReal)*maxwork);
    ywork= (GpReal*)p_malloc(sizeof(GpReal)*maxwork);
  }
  xClip= xwork;
  yClip= ywork;
